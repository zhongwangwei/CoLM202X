...
def test_dissolved_limit_precipitates_and_redissolves_without_losing_mass(
    ...
    assert result.returncode == 0, result.stdout + result.stderr
    lines = result.stdout.splitlines()
    dry = [float(value) for value in next(line for line in lines if line.startswith("DRY=")).removeprefix("DRY=").split()]
    rewet = [float(value) for value in next(line for line in lines if line.startswith("REWET=")).removeprefix("REWET=").split()]
