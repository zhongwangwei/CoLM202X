...
MODULE MOD_Tracer_Methane_Coupler
   ...
   PUBLIC :: tracer_methane_resolve_step
   PUBLIC :: tracer_methane_lake_step
   PUBLIC :: tracer_methane_wetland_decomp
   PUBLIC :: tracer_methane_soil_step
   PUBLIC :: tracer_methane_report

CONTAINS

   SUBROUTINE tracer_methane_resolve_step (istep_in, istep_local)
      ...
      ELSE
         ! DA path or other callers: behave as first timestep.
         istep_local = 1
      ENDIF

   END SUBROUTINE tracer_methane_resolve_step

   SUBROUTINE tracer_methane_lake_step (istep_local, ipatch, idate, deltim_phy, isub, nsub)
   ...
   END SUBROUTINE tracer_methane_lake_step

   SUBROUTINE tracer_methane_wetland_decomp (ipatch)
   ...
   END SUBROUTINE tracer_methane_wetland_decomp

   SUBROUTINE tracer_methane_soil_step (istep_local, ipatch, idate, deltim)
   ...
   END SUBROUTINE tracer_methane_soil_step

   SUBROUTINE tracer_methane_report ()

      IMPLICIT NONE

      IF (igas_ch4 > 0) CALL tracer_balance_report()

   END SUBROUTINE tracer_methane_report
...
END MODULE MOD_Tracer_Methane_Coupler
#endif
