...
!=======================================================================
   ...
   !!!! --------------------------------------------------------------------------------------------------------
	...
	real(r8), allocatable :: co2_decomp_depth    (:,:) ! CO2 production from decomposition/methanogenesis (mol/m3/s)
	...
	real(r8), allocatable :: methane_surf_flux_tot     (:) ! CH4 flux to atm. (mol/m2/s)
	...
	real(r8), allocatable :: methane_surf_diff         (:) ! Output: CH4 surface flux (mol/m2/s)
	...
	real(r8), allocatable :: co2_decomp_tot       (:) ! total CO2 from decomposition/methanogenesis (mol/m2/s)
   ...
   real(r8), allocatable :: co2_decomp_depth_unsat    (:,:)  ! CO2 production from decomposition/methanogenesis (unsat) (mol/m3/s)
   real(r8), allocatable :: co2_decomp_depth_sat      (:,:)  ! CO2 production from decomposition/methanogenesis (sat)   (mol/m3/s)
   ...
   real(r8), allocatable :: lulcc_f_h2osfc_old(:), lulcc_forc_pmethanem_old(:)
	   ...
	   integer, parameter :: methane_lake_substep_n1d = 42
   ...
   PRIVATE :: clear_methane_lulcc_snapshot
   PRIVATE :: initialize_methane_lake_soilc_from_surface
...
CONTAINS
   ...
   SUBROUTINE allocate_methane_state (numpatch)
      ...
      allocate (conc_o2             (nl_soil,numpatch)); conc_o2              (:,:) = 0._r8
      allocate (conc_methane            (nl_soil,numpatch)); conc_methane             (:,:) = 0._r8
      !!!! --------------------------------------------------------------------------------------------------------
      ...
      allocate (annavg_finrw                (numpatch)); annavg_finrw           (:) = 0._r8
   ...
   SUBROUTINE read_methane_restart (file_restart)
      ...
      CALL ncio_read_vector (file_restart, 'ch4_annavg_finrw',     landpatch, annavg_finrw,     defval = 0._r8)
      ...
      WHERE (invalid_restart_value(totcol_methane)       .or. &
             invalid_restart_value(totcol_methane_unsat) .or. &
             invalid_restart_value(totcol_methane_sat)   .or. &
             invalid_restart_value(totcol_methane_lake))
         ! Backward compatibility for restart files written before CH4 column
         ! inventories were persisted.  Suppress the first-step balance check
         ! only for patches whose restored column inventory was invalid.
         fsat_bef = spval
      END WHERE
   ...
   SUBROUTINE initialize_methane_lake_soilc_from_surface (patchtype_in, lake_soilc_srf_in, allowlakeprod)
      ...
      DO ipatch = 1, npatch
         ...
         IF (sum(max(lake_soilc_srf_in(1:nl_soil,ipatch), 0._r8)) <= smallnumber) CYCLE
