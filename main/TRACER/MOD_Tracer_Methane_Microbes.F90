   ...
   SUBROUTINE methane_microbes_step(ipatch, deltim, t_soisno, conc_o2, conc_ch4, hr_vr, cellorg)
      ...
      DO j = 1, nl_soil
	         IF (t_soisno(j) <= tfrz) THEN
	            ...
	            B_methanogen(j,ipatch) = max(0._r8, &
	               B_methanogen(j,ipatch) * (1._r8 - freeze_loss))
	            B_methanotroph(j,ipatch) = max(0._r8, &
         ...
         ! cellorg is [kg OM m-3]; convert to [mol C m-3] using 0.5 gC/gOM.
         sub_pool = max(cellorg(j), 0._r8) * 1000._r8 * 0.5_r8 / catomw
         ...
         prod_pot = DEF_METHANE%kappa_m_methanogen * B_methanogen(j,ipatch) * &
            tempfac * f_s * f_o2 / secspday
         carbon_cap = sub_rate
         prod_pot = min(max(prod_pot, 0._r8), carbon_cap)

         oxid_pot = DEF_METHANE%kappa_m_methanotroph * B_methanotroph(j,ipatch) * &
