#include <define.h>

MODULE MOD_Tracer_Forcing

!-----------------------------------------------------------------------
! !DESCRIPTION:
!  read in the atmospheric forcing using user defined interpolation method or
!  downscaling forcing
!
! !REVISIONS:
!  Yongjiu Dai and Hua Yuan, 04/2014: initial code from CoLM2014 (tracerdata.F90,
!                                     GETMET.F90 and rd_forcing.F90
!
!  Shupeng Zhang, 05/2023: 1) porting codes to MPI parallel version
!                          2) codes for dealing with missing forcing value
!                          3) interface for downscaling
!
! !TODO...(need complement)
!-----------------------------------------------------------------------

   USE MOD_Precision
   USE MOD_Namelist
   USE MOD_Grid
   USE MOD_SpatialMapping
   USE MOD_UserSpecifiedForcing
   USE MOD_TimeManager
   USE MOD_SPMD_Task
   USE MOD_MonthlyinSituCO2MaunaLoa
   USE MOD_Vars_Global, only: pi
   USE MOD_OrbCoszen
   USE MOD_UserDefFun
   USE MOD_Tracer_Namelist_Defs, only: DEF_Tracers, nl_tracer_forcing_type, DEF_Tracer_Forcings_NL, MAX_TRACER_FORCING_VARS

   IMPLICIT NONE

   type (grid_type), PUBLIC :: gforc_tracer

   type (spatial_mapping_type) :: mg2p_forc_tracer   ! area weighted mapping from forcing to model unit

   logical, allocatable :: forcmask_pch_tracer (:)

   ! for Forcing_Downscaling
   type(block_data_real8_2d) :: topo_grid_tracer, maxelv_grid_tracer
   type(block_data_real8_2d) :: sumarea_grid_tracer

   type(pointer_real8_1d), allocatable :: forc_topo_grid_tracer   (:)
   type(pointer_real8_1d), allocatable :: forc_maxelv_grid_tracer (:)

   type(pointer_real8_1d), allocatable :: forc_q_grid_O18   (:)
   type(pointer_real8_1d), allocatable :: forc_prc_grid_O18  (:)
   type(pointer_real8_1d), allocatable :: forc_prl_grid_O18  (:)
   type(pointer_real8_1d), allocatable :: forc_q_part_O18     (:)
   type(pointer_real8_1d), allocatable :: forc_prc_part_O18   (:)
   type(pointer_real8_1d), allocatable :: forc_prl_part_O18   (:)

   type(pointer_real8_1d), allocatable :: forc_q_grid_H2    (:)
   type(pointer_real8_1d), allocatable :: forc_prc_grid_H2  (:)
   type(pointer_real8_1d), allocatable :: forc_prl_grid_H2  (:)
   type(pointer_real8_1d), allocatable :: forc_q_part_H2     (:)
   type(pointer_real8_1d), allocatable :: forc_prc_part_H2   (:)
   type(pointer_real8_1d), allocatable :: forc_prl_part_H2   (:)

   logical, allocatable :: glacierss_tracer (:)

   ! local variables
   integer  :: deltim_int_tracer                ! model time step length
   ! real(r8) :: deltim_real             ! model time step length

   !  for SinglePoint
   type(timestamp), allocatable :: forctime_tracer (:)
   integer,  allocatable :: iforctime_tracer(:)

   logical :: tracer_read_ahead
   real(r8), allocatable :: tracer_disk(:,:)

   type(timestamp), allocatable :: tstamp_LB_tracer(:)  ! time stamp of low boundary data
   type(timestamp), allocatable :: tstamp_UB_tracer(:)  ! time stamp of up boundary data

   type(block_data_real8_2d) :: avgcos_tracer   ! time-average of cos(zenith)
   type(block_data_real8_2d) :: tracerdata  ! forcing data
#ifdef URBAN_MODEL
   type(block_data_real8_2d) :: rainf_tracer
   type(block_data_real8_2d) :: snowf_tracer
#endif

   type(block_data_real8_2d), allocatable :: forcn_tracer    (:)  ! forcing data
   type(block_data_real8_2d), allocatable :: forcn_tracer_LB (:)  ! forcing data at lower boundary
   type(block_data_real8_2d), allocatable :: forcn_tracer_UB (:)  ! forcing data at upper boundary




   character(len=64), allocatable :: tracer_name(:)       ! tracer name
   character(len=256), allocatable :: tracer_dataset_name(:)      ! dataset name
   character(len=16), allocatable :: tracer_type(:)        ! tracer type
   character(len=256), allocatable :: tracer_dir(:)        ! tracer directory
   integer, allocatable :: tracer_startyr(:)                      ! start year
   integer, allocatable :: tracer_startmo(:)                      ! start month
   integer, allocatable :: tracer_endyr(:)                        ! end year
   integer, allocatable :: tracer_endmo(:)                        ! end month
   integer, allocatable :: tracer_dtime(:)                        ! time step
   integer, allocatable :: tracer_offset(:)                        ! tracer_offset
 
   logical, allocatable :: tracer_leapyear(:)                          ! leap year
   logical, allocatable :: tracer_data2d(:)                            ! 2d data
   logical, allocatable :: tracer_hightdim(:)                          ! height dimension
   logical, allocatable :: tracer_dim2d(:)                             ! 2d dimension
   character(len=256), allocatable :: tracer_latname(:)               ! latitude name
   character(len=256), allocatable :: tracer_lonname(:)               ! longitude name
   character(len=256), allocatable :: tracer_groupby(:)               ! group by
   integer, allocatable :: tracer_NVAR(:)                              ! NVAR
   logical, allocatable :: tracer_has_missing_value(:)                 ! has missing value
   character(len=256), allocatable :: tracer_missing_value_name(:)                ! missing value name
 
   logical, allocatable :: tracer_regional(:)                          ! tracer_regional
   real(r8), allocatable :: tracer_regbnd(:)                         ! region boundary
 
   character(len=256), allocatable :: tracer_fprefix(:)  ! file prefix
   character(len=256), allocatable :: tracer_vname(:)    ! variable name
   character(len=256), allocatable :: tracer_timelog(:)  ! time log info
   character(len=256), allocatable :: tracer_tintalgo(:) ! interpolation algorithm

CONTAINS

!-----------------------------------------------------------------------
   SUBROUTINE tracer_forcing_init (deltatime, ststamp, lc_year, etstamp, lulcc_call)

   USE MOD_SPMD_Task
   USE MOD_Namelist
   USE MOD_Block
   USE MOD_DataType
   USE MOD_Mesh
   USE MOD_LandElm
   USE MOD_LandPatch
#ifdef CROP
   USE MOD_LandCrop
#endif
   USE MOD_UserSpecifiedForcing
   USE MOD_NetCDFSerial
   USE MOD_NetCDFVector
   USE MOD_NetCDFBlock
   USE MOD_Vars_TimeINVAR_traceriants
   USE MOD_Vars_1DForcing
   IMPLICIT NONE

   real(r8),         intent(in) :: deltatime  ! model time step
   type(timestamp),  intent(in) :: ststamp
   integer,          intent(in) :: lc_year    ! which year of land cover data used
   type(timestamp),  intent(in), optional :: etstamp
   logical,          intent(in), optional :: lulcc_call ! whether it is a lulcc CALL

   ! Local variables
   integer            :: idate(3)
   type(timestamp)    :: tstamp
   character(len=256) :: filename, lndname, cyear
   integer            :: ivar, year, month, day, time_i
   real(r8)           :: missing_value
   integer            :: ielm, istt, iend

   integer :: iblkme, xblk, yblk, xloc, yloc, NVAR_tracer, num_tracers_with_forcing
   
   ! Check allocation status of tracer arrays
   num_tracers_with_forcing = SIZE(DEF_Tracer_Forcings_NL, 1)
   NVAR_tracer = 0
   num_tracers_with_forcing = 0
   DO k = 1, num_tracers_with_forcing
         NVAR_tracer = DEF_Tracer_Forcings_NL(k)%NVAR+NVAR_tracer
   ENDDO
   write(*,*) "NVAR_tracer", NVAR_tracer

   ! allocate memory for tracer arrays
   allocate (tracer_name(NVAR_tracer))
   allocate (tracer_dataset_name(NVAR_tracer))
   allocate (tracer_type(NVAR_tracer))
   allocate (tracer_dir(NVAR_tracer))
   allocate (tracer_startyr(NVAR_tracer))
   allocate (tracer_startmo(NVAR_tracer))
   allocate (tracer_endyr(NVAR_tracer))
   allocate (tracer_endmo(NVAR_tracer))
   allocate (tracer_dtime(NVAR_tracer))
   allocate (tracer_offset(NVAR_tracer))
   allocate (tracer_leapyear(NVAR_tracer))
   allocate (tracer_data2d(NVAR_tracer))
   allocate (tracer_hightdim(NVAR_tracer))
   allocate (tracer_dim2d(NVAR_tracer))
   allocate (tracer_groupby(NVAR_tracer))
   allocate (tracer_has_missing_value(NVAR_tracer))
   allocate (tracer_missing_value_name(NVAR_tracer))
   allocate (tracer_regional(NVAR_tracer))
   allocate (tracer_regbnd(NVAR_tracer))
   allocate (tracer_fprefix(NVAR_tracer))
   allocate (tracer_vname(NVAR_tracer))
   allocate (tracer_timelog(NVAR_tracer))
   allocate (tracer_tintalgo(NVAR_tracer))
   ! set initial values
   num=1
   DO k = 1, num_tracers_with_forcing
      tracer_name(num) = DEF_Tracer_Forcings_NL(k)%tracer_name
      tracer_dataset_name(num) = DEF_Tracer_Forcings_NL(k)%dataset_name
      tracer_type(num) = DEF_Tracer_Forcings_NL(k)%tracer_type
      tracer_dir(num) = DEF_Tracer_Forcings_NL(k)%tracer_dir
      tracer_startyr(num) = DEF_Tracer_Forcings_NL(k)%startyr
      tracer_startmo(num) = DEF_Tracer_Forcings_NL(k)%startmo
      tracer_endyr(num) = DEF_Tracer_Forcings_NL(k)%endyr
      tracer_endmo(num) = DEF_Tracer_Forcings_NL(k)%endmo
      tracer_dtime(num) = DEF_Tracer_Forcings_NL(k)%dtime
      tracer_offset(num) = DEF_Tracer_Forcings_NL(k)%offset
      tracer_leapyear(num) = DEF_Tracer_Forcings_NL(k)%leapyear
      tracer_data2d(num) = DEF_Tracer_Forcings_NL(k)%data2d
      tracer_hightdim(num) = DEF_Tracer_Forcings_NL(k)%hightdim
      tracer_dim2d(num) = DEF_Tracer_Forcings_NL(k)%dim2d
      tracer_groupby(num) = DEF_Tracer_Forcings_NL(k)%groupby
      tracer_has_missing_value(num) = DEF_Tracer_Forcings_NL(k)%has_missing_value
      tracer_missing_value_name(num) = DEF_Tracer_Forcings_NL(k)%missing_value_name
      tracer_regional(num) = DEF_Tracer_Forcings_NL(k)%regional
      tracer_regbnd(num) = DEF_Tracer_Forcings_NL(k)%regbnd
      tracer_fprefix(num) = DEF_Tracer_Forcings_NL(k)%fprefix
      tracer_vname(num) = DEF_Tracer_Forcings_NL(k)%vname
      tracer_timelog(num) = DEF_Tracer_Forcings_NL(k)%timelog
      tracer_tintalgo(num) = DEF_Tracer_Forcings_NL(k)%tintalgo
      num=num+1
   ENDDO

   ! get value of fmetdat and deltim
   deltim_int_tracer  = int(deltatime)
   ! deltim_real = deltatime

   ! set initial values
   IF (allocated(tstamp_LB_tracer)) deallocate(tstamp_LB_tracer)
   IF (allocated(tstamp_UB_tracer)) deallocate(tstamp_UB_tracer)
   allocate (tstamp_LB_tracer(NVAR_tracer))
   allocate (tstamp_UB_tracer(NVAR_tracer))
   tstamp_LB_tracer(:) = timestamp(-1, -1, -1)
   tstamp_UB_tracer(:) = timestamp(-1, -1, -1)

   idate = (/ststamp%year, ststamp%day, ststamp%sec/)

   CALL adj2begin (idate)
   ! %assume they have the same lon lat
   CALL tracer_read_latlon (DEF_Tracer_Forcings_NL(1)%tracer_dir, idate) 

   IF (p_is_io) THEN
         IF (allocated(forcn_tracer   )) deallocate(forcn_tracer   )
         IF (allocated(forcn_tracer_LB)) deallocate(forcn_tracer_LB)
         IF (allocated(forcn_tracer_UB)) deallocate(forcn_tracer_UB)
         allocate (forcn_tracer    (NVAR_tracer))
         allocate (forcn_tracer_LB (NVAR_tracer))
         allocate (forcn_tracer_UB (NVAR_tracer))

         DO ivar = 1, NVAR_tracer
            CALL allocate_block_data (gforc_tracer, forcn_tracer   (ivar))
            CALL allocate_block_data (gforc_tracer, forcn_tracer_LB(ivar))
            CALL allocate_block_data (gforc_tracer, forcn_tracer_UB(ivar))
         ENDDO

         ! allocate memory for forcing data
         CALL allocate_block_data (gforc_tracer, tracerdata)  ! forcing data
         CALL allocate_block_data (gforc_tracer, avgcos_tracer )  ! time-average of cos(zenith)
#if (defined URBAN_MODEL && defined SinglePoint)
         CALL allocate_block_data (gforc_tracer, rainf_tracer)
         CALL allocate_block_data (gforc_tracer, snowf_tracer)
#endif

      ENDIF

      IF (p_is_worker) THEN
         IF (numpatch > 0) THEN
            allocate (forcmask_pch_tracer(numpatch));  forcmask_pch_tracer(:) = .true.
         ENDIF
      ENDIF

      IF (DEF_Tracer_Forcings_NL(1)%tracer_has_missing_value) THEN

         tstamp = idate
         CALL setstampLB(tstamp, 1, year, month, day, time_i)
         filename = trim(dir_forcing)//trim(metfilename(year, month, day, 1))
         tstamp_LB_tracer(1) = timestamp(-1, -1, -1)

         IF (p_is_master) THEN
            CALL ncio_get_attr (filename, tracer_vname(1), trim(DEF_forcing%tracer_missing_value_name), &
                                missing_value)
         ENDIF
#ifdef USEMPI
         CALL mpi_bcast (missing_value, 1, MPI_REAL8, p_address_master, p_comm_glb, p_err)
#endif

         CALL ncio_read_block_time (filename, tracer_vname(1), gforc_tracer, time_i, tracerdata)

      ENDIF

      IF (trim(DEF_Forcing_Interp_Method) == 'arealweight') THEN
         IF (present(lulcc_call)) CALL mg2p_forc_tracer%forc_free_mem
         CALL mg2p_forc_tracer%build_arealweighted (gforc_tracer, landpatch)
      ELSEIF (trim(DEF_Forcing_Interp_Method) == 'bilinear') THEN
         IF (present(lulcc_call)) CALL mg2p_forc_tracer%forc_free_mem
         CALL mg2p_forc_tracer%build_bilinear (gforc_tracer, landpatch)
      ENDIF

      IF (DEF_forcing%tracer_has_missing_value) THEN
         CALL mg2p_forc_tracer%set_missing_value (tracerdata, missing_value, forcmask_pch_tracer)
      ENDIF

      IF (DEF_USE_Forcing_Downscaling) THEN

         IF (p_is_worker .and. (numpatch > 0)) THEN
            forc_topo = topoelv
            WHERE(forc_topo == spval) forc_topo = 0.
         ENDIF

         IF (p_is_io) CALL allocate_block_data (gforc_tracer, topo_grid)
         CALL mg2p_forc_tracer%pset2grid (forc_topo, topo_grid)

         IF (p_is_io) CALL allocate_block_data (gforc_tracer, sumarea_grid)
         CALL mg2p_forc_tracer%get_sumarea (sumarea_grid)

         CALL block_data_division (topo_grid, sumarea_grid)

         IF (p_is_io) CALL allocate_block_data (gforc_tracer, maxelv_grid)
         CALL mg2p_forc_tracer%pset2grid_max (forc_topo, maxelv_grid)


         CALL mg2p_forc_tracer%allocate_part (forc_topo_grid  )
         CALL mg2p_forc_tracer%allocate_part (forc_maxelv_grid)

         CALL mg2p_forc_tracer%allocate_part (forc_t_grid     )
         CALL mg2p_forc_tracer%allocate_part (forc_th_grid    )
         CALL mg2p_forc_tracer%allocate_part (forc_q_grid     )
         CALL mg2p_forc_tracer%allocate_part (forc_pbot_grid  )
         CALL mg2p_forc_tracer%allocate_part (forc_rho_grid   )
         CALL mg2p_forc_tracer%allocate_part (forc_prc_grid   )
         CALL mg2p_forc_tracer%allocate_part (forc_prl_grid   )
         CALL mg2p_forc_tracer%allocate_part (forc_lwrad_grid )
         CALL mg2p_forc_tracer%allocate_part (forc_swrad_grid )
         CALL mg2p_forc_tracer%allocate_part (forc_hgt_grid   )
         CALL mg2p_forc_tracer%allocate_part (forc_us_grid    )
         CALL mg2p_forc_tracer%allocate_part (forc_vs_grid    )

         CALL mg2p_forc_tracer%allocate_part (forc_t_part     )
         CALL mg2p_forc_tracer%allocate_part (forc_th_part    )
         CALL mg2p_forc_tracer%allocate_part (forc_q_part     )
         CALL mg2p_forc_tracer%allocate_part (forc_pbot_part  )
         CALL mg2p_forc_tracer%allocate_part (forc_rhoair_part)
         CALL mg2p_forc_tracer%allocate_part (forc_prc_part   )
         CALL mg2p_forc_tracer%allocate_part (forc_prl_part   )
         CALL mg2p_forc_tracer%allocate_part (forc_frl_part   )
         CALL mg2p_forc_tracer%allocate_part (forc_swrad_part )
         CALL mg2p_forc_tracer%allocate_part (forc_us_part    )
         CALL mg2p_forc_tracer%allocate_part (forc_vs_part    )

         CALL mg2p_forc_tracer%grid2part (topo_grid,   forc_topo_grid  )
         CALL mg2p_forc_tracer%grid2part (maxelv_grid, forc_maxelv_grid)

         IF (p_is_worker .and. (numpatch > 0)) THEN
            allocate (glacierss(numpatch))
            glacierss(:) = patchtype(:) == 3
         ENDIF

      ENDIF

      tracer_read_ahead = .false.
      IF (trim(DEF_forcing%dataset) == 'POINT') THEN
         IF (USE_SITE_ForcingReadAhead .and. present(etstamp)) THEN
            tracer_read_ahead = .true.
            CALL metread_time (dir_forcing, ststamp, etstamp, deltatime)
         ELSE
            CALL metread_time (dir_forcing)
         ENDIF
         allocate (iforctime(NVAR_tracer))
      ENDIF

      IF (trim(DEF_forcing%dataset) == 'POINT') THEN

         filename = trim(dir_forcing)//trim(tracer_fprefix(1))

#ifndef URBAN_MODEL
         IF (ncio_var_exist(filename,'reference_height_v')) THEN
            CALL ncio_read_serial (filename, 'reference_height_v', Height_V)
         ENDIF

         IF (ncio_var_exist(filename,'reference_height_t')) THEN
            CALL ncio_read_serial (filename, 'reference_height_t', Height_T)
         ENDIF

         IF (ncio_var_exist(filename,'reference_height_q')) THEN
            CALL ncio_read_serial (filename, 'reference_height_q', Height_Q)
         ENDIF
#else
         IF (ncio_var_exist(filename,'measurement_height_above_ground')) THEN
            CALL ncio_read_serial (filename, 'measurement_height_above_ground', Height_V)
            CALL ncio_read_serial (filename, 'measurement_height_above_ground', Height_T)
            CALL ncio_read_serial (filename, 'measurement_height_above_ground', Height_Q)
         ENDIF
#endif

      ENDIF

   END SUBROUTINE tracer_forcing_init

   ! ---- forcing finalize ----
   SUBROUTINE forcing_final ()

   USE MOD_LandPatch, only: numpatch
   IMPLICIT NONE

      IF (allocated(forcmask_pch_tracer)) deallocate(forcmask_pch_tracer)
      IF (allocated(glacierss   )) deallocate(glacierss   )
      IF (allocated(forctime    )) deallocate(forctime    )
      IF (allocated(iforctime   )) deallocate(iforctime   )
      IF (allocated(tracer_disk   )) deallocate(tracer_disk   )
      IF (allocated(tstamp_LB_tracer   )) deallocate(tstamp_LB_tracer   )
      IF (allocated(tstamp_UB_tracer   )) deallocate(tstamp_UB_tracer   )

      IF (DEF_USE_Forcing_Downscaling) THEN
         IF (p_is_worker) THEN
            IF (numpatch > 0) THEN

               deallocate (forc_topo_grid  )
               deallocate (forc_maxelv_grid)

               deallocate (forc_t_grid     )
               deallocate (forc_th_grid    )
               deallocate (forc_q_grid     )
               deallocate (forc_pbot_grid  )
               deallocate (forc_rho_grid   )
               deallocate (forc_prc_grid   )
               deallocate (forc_prl_grid   )
               deallocate (forc_lwrad_grid )
               deallocate (forc_swrad_grid )
               deallocate (forc_hgt_grid   )

               deallocate (forc_t_part     )
               deallocate (forc_th_part    )
               deallocate (forc_q_part     )
               deallocate (forc_pbot_part  )
               deallocate (forc_rhoair_part)
               deallocate (forc_prc_part   )
               deallocate (forc_prl_part   )
               deallocate (forc_frl_part   )
               deallocate (forc_swrad_part )

            ENDIF
         ENDIF
      ENDIF

   END SUBROUTINE forcing_final

   ! ------------
   SUBROUTINE forcing_reset ()

   IMPLICIT NONE

      tstamp_LB_tracer(:) = timestamp(-1, -1, -1)
      tstamp_UB_tracer(:) = timestamp(-1, -1, -1)

   END SUBROUTINE forcing_reset


!-----------------------------------------------------------------------
   SUBROUTINE read_tracer_forcing (idate, dir_forcing)
   USE MOD_OrbCosazi
   USE MOD_Precision
   USE MOD_Namelist
   USE MOD_Const_Physical, only: rgas, grav
   USE MOD_Vars_TimeINVAR_traceriants
   USE MOD_Vars_TimeVariables, only: alb
   USE MOD_Vars_1DForcing
   USE MOD_Vars_2DForcing
   USE MOD_Block
   USE MOD_SPMD_Task
   USE MOD_DataType
   USE MOD_Mesh
   USE MOD_LandPatch
   USE MOD_RangeCheck
   USE MOD_UserSpecifiedForcing
   USE MOD_ForcingDownscaling, only: rair, cpair, downscale_forcings, downscale_wind
   USE MOD_NetCDFVector

   IMPLICIT NONE

   integer, intent(in) :: idate(3)
   character(len=*), intent(in) :: dir_forcing

   ! local variables:
   integer  :: ivar, istt, iend, id(3)
   integer  :: iblkme, ib, jb, i, j, ilon, ilat, np, ipart, ne
   real(r8) :: calday                             ! Julian cal day (1.xx to 365.xx)
   real(r8) :: sunang, cloud, difrat, vnrat
   real(r8) :: a, hsolar, ratio_rvrf
   type(block_data_real8_2d) :: forc_xy_solarin
   integer  :: ii
   character(10) :: cyear = "2005"
   character(256):: lndname

   type(timestamp) :: mtstamp
   integer  :: dtLB, dtUB
   real(r8) :: cosz, coszen(numpatch), cosa, cosazi(numpatch), balb
   integer  :: year, month, mday
   logical  :: has_u,has_v
   real solar, frl, prcp, tm, us, vs, pres, qm
   real(r8) :: pco2m
   real(r8), dimension(12, numpatch) :: spaceship !NOTE: 12 is the dimension size of spaceship
   integer target_server, ierr

      IF (p_is_io) THEN
         !------------------------------------------------------------
         ! READ in THE ATMOSPHERIC FORCING
         ! read lower and upper boundary forcing data
         CALL metreadLBUB(idate, dir_forcing)
         ! set model time stamp
         id(:) = idate(:)
         !CALL adj2end(id)
         mtstamp = id
         has_u = .true.
         has_v = .true.
         ! loop for variables
         DO ivar = 1, NVAR_tracer
            IF (ivar == 5 .and. trim(tracer_vname(ivar)) == 'NULL') has_u = .false.
            IF (ivar == 6 .and. trim(tracer_vname(ivar)) == 'NULL') has_v = .false.
            IF (trim(tracer_vname(ivar)) == 'NULL') CYCLE     ! no data, CYCLE
            IF (trim(tracer_tintalgo(ivar)) == 'NULL') CYCLE

            ! to make sure the forcing data calculated is in the range of time
            ! interval [LB, UB]
            IF ( (mtstamp < tstamp_LB_tracer(ivar)) .or. (tstamp_UB_tracer(ivar) < mtstamp) ) THEN
               write(6, *) "the data required is out of range! STOP!"; CALL CoLM_stop()
            ENDIF

            ! calculate distance to lower/upper boundary
            dtLB = mtstamp - tstamp_LB_tracer(ivar)
            dtUB = tstamp_UB_tracer(ivar) - mtstamp

            ! linear method, for T, Pres, Q, W, LW
            IF (tracer_tintalgo(ivar) == 'linear') THEN
               IF ( (dtLB+dtUB) > 0 ) THEN
                  CALL block_data_linear_interp ( &
                     forcn_tracer_LB(ivar), real(dtUB,r8)/real(dtLB+dtUB,r8), &
                     forcn_tracer_UB(ivar), real(dtLB,r8)/real(dtLB+dtUB,r8), &
                     forcn_tracer(ivar))
               ELSE
                  CALL block_data_copy (forcn_tracer_LB(ivar), forcn_tracer(ivar))
               ENDIF
            ENDIF

            ! for precipitation, two algorithms available
            ! nearest method, for precipitation
            IF (tracer_tintalgo(ivar) == 'nearest') THEN
               IF (dtLB <= dtUB) THEN
                  CALL block_data_copy (forcn_tracer_LB(ivar), forcn_tracer(ivar))
               ELSE
                  CALL block_data_copy (forcn_tracer_UB(ivar), forcn_tracer(ivar))
               ENDIF
            ENDIF

            ! set all the same value, for precipitation
            IF (tracer_tintalgo(ivar) == 'uniform') THEN
               IF (trim(tracer_timelog(ivar)) == 'forward') THEN
                  CALL block_data_copy (forcn_tracer_LB(ivar), forcn_tracer(ivar))
               ELSE
                  CALL block_data_copy (forcn_tracer_UB(ivar), forcn_tracer(ivar))
               ENDIF
            ENDIF

            ! coszen method, for SW
            IF (tracer_tintalgo(ivar) == 'coszen') THEN
               DO iblkme = 1, gblock%nblkme
                  ib = gblock%xblkme(iblkme)
                  jb = gblock%yblkme(iblkme)

                  DO j = 1, gforc_tracer%ycnt(jb)
                     DO i = 1, gforc_tracer%xcnt(ib)

                        ilat = gforc_tracer%ydsp(jb) + j
                        ilon = gforc_tracer%xdsp(ib) + i
                        IF (ilon > gforc_tracer%nlon) ilon = ilon - gforc_tracer%nlon

                        calday = calendarday(mtstamp)
                        cosz = orb_coszen(calday, gforc_tracer%rlon(ilon), gforc_tracer%rlat(ilat))
                        cosz = max(0.001, cosz)
                        ! 10/24/2024, yuan: deal with time log with backward or forward
                        IF (trim(tracer_timelog(ivar)) == 'forward') THEN
                           forcn_tracer(ivar)%blk(ib,jb)%val(i,j) = &
                              cosz / avgcos_tracer%blk(ib,jb)%val(i,j) * forcn_tracer_LB(ivar)%blk(ib,jb)%val(i,j)
                        ELSE
                           forcn_tracer(ivar)%blk(ib,jb)%val(i,j) = &
                              cosz / avgcos_tracer%blk(ib,jb)%val(i,j) * forcn_tracer_UB(ivar)%blk(ib,jb)%val(i,j)
                        ENDIF

                     ENDDO
                  ENDDO
               ENDDO
            ENDIF

         ENDDO

         ! preprocess for forcing data, only for QIAN data right now?
         CALL metpreprocess (gforc_tracer, forcn_tracer)

         CALL allocate_block_data (gforc_tracer, forc_xy_solarin)

         CALL block_data_copy (forcn_tracer(1), forc_xy_t      )
         CALL block_data_copy (forcn_tracer(2), forc_xy_q      )
         CALL block_data_copy (forcn_tracer(3), forc_xy_psrf   )
         CALL block_data_copy (forcn_tracer(3), forc_xy_pbot   )
         CALL block_data_copy (forcn_tracer(4), forc_xy_prl, sca = 2/3._r8)
         CALL block_data_copy (forcn_tracer(4), forc_xy_prc, sca = 1/3._r8)
         CALL block_data_copy (forcn_tracer(7), forc_xy_solarin)
         CALL block_data_copy (forcn_tracer(8), forc_xy_frl    )
         IF (DEF_USE_CBL_HEIGHT) THEN
         CALL block_data_copy (forcn_tracer(9), forc_xy_hpbl   )
         ENDIF

         IF (has_u .and. has_v) THEN
            CALL block_data_copy (forcn_tracer(5), forc_xy_us )
            CALL block_data_copy (forcn_tracer(6), forc_xy_vs )
         ELSEIF (has_u) THEN
            CALL block_data_copy (forcn_tracer(5), forc_xy_us , sca = 1/sqrt(2.0_r8))
            CALL block_data_copy (forcn_tracer(5), forc_xy_vs , sca = 1/sqrt(2.0_r8))
         ELSEIF (has_v) THEN
            CALL block_data_copy (forcn_tracer(6), forc_xy_us , sca = 1/sqrt(2.0_r8))
            CALL block_data_copy (forcn_tracer(6), forc_xy_vs , sca = 1/sqrt(2.0_r8))
         ELSE
            IF (.not.trim(DEF_forcing%dataset) == 'CPL7') THEN
               write(6, *) "At least one of the wind components must be provided! STOP!";
            CALL CoLM_stop()
            ENDIF
         ENDIF

         CALL flush_block_data (forc_xy_hgt_u, real(HEIGHT_V,r8))
         CALL flush_block_data (forc_xy_hgt_t, real(HEIGHT_T,r8))
         CALL flush_block_data (forc_xy_hgt_q, real(HEIGHT_Q,r8))

         IF (solarin_all_band) THEN

            IF (trim(DEF_forcing%dataset) == 'QIAN') THEN
               !---------------------------------------------------------------
               ! 04/2014, yuan: NOTE! codes from CLM4.5-CESM1.2.0
               ! relationship between incoming NIR or VIS radiation and ratio of
               ! direct to diffuse radiation calculated based on one year's worth of
               ! hourly CAM output from CAM version cam3_5_55
               !---------------------------------------------------------------
               DO iblkme = 1, gblock%nblkme
                  ib = gblock%xblkme(iblkme)
                  jb = gblock%yblkme(iblkme)

                  DO j = 1, gforc_tracer%ycnt(jb)
                     DO i = 1, gforc_tracer%xcnt(ib)

                        hsolar = forc_xy_solarin%blk(ib,jb)%val(i,j)*0.5_R8

                        ! NIR (dir, diff)
                        ratio_rvrf = min(0.99_R8,max(0.29548_R8 + 0.00504_R8*hsolar  &
                           -1.4957e-05_R8*hsolar**2 + 1.4881e-08_R8*hsolar**3,0.01_R8))
                        forc_xy_soll %blk(ib,jb)%val(i,j) = ratio_rvrf*hsolar
                        forc_xy_solld%blk(ib,jb)%val(i,j) = (1._R8 - ratio_rvrf)*hsolar

                        ! VIS (dir, diff)
                        ratio_rvrf = min(0.99_R8,max(0.17639_R8 + 0.00380_R8*hsolar  &
                           -9.0039e-06_R8*hsolar**2 + 8.1351e-09_R8*hsolar**3,0.01_R8))
                        forc_xy_sols %blk(ib,jb)%val(i,j) = ratio_rvrf*hsolar
                        forc_xy_solsd%blk(ib,jb)%val(i,j) = (1._R8 - ratio_rvrf)*hsolar

                     ENDDO
                  ENDDO
               ENDDO

            ELSE
               !---------------------------------------------------------------
               ! as the downward solar is in full band, an empirical expression
               ! will be used to divide fractions of band and incident
               ! (visible, near-infrad, dirct, diffuse)
               ! Julian calday (1.xx to 365.xx)
               !---------------------------------------------------------------
               DO iblkme = 1, gblock%nblkme
                  ib = gblock%xblkme(iblkme)
                  jb = gblock%yblkme(iblkme)

                  DO j = 1, gforc_tracer%ycnt(jb)
                     DO i = 1, gforc_tracer%xcnt(ib)

                        ilat = gforc_tracer%ydsp(jb) + j
                        ilon = gforc_tracer%xdsp(ib) + i
                        IF (ilon > gforc_tracer%nlon) ilon = ilon - gforc_tracer%nlon

                        a = max(0., forc_xy_solarin%blk(ib,jb)%val(i,j))
                        calday = calendarday(idate)
                        sunang = orb_coszen (calday, gforc_tracer%rlon(ilon), gforc_tracer%rlat(ilat))

                        IF (sunang .eq. 0)THEN
                           cloud = 0.
                        ELSE
                           cloud = (1160.*sunang-a)/(963.*sunang)
                        ENDIF
                        cloud = max(cloud,0.)
                        cloud = min(cloud,1.)
                        cloud = max(0.58,cloud)

                        difrat = 0.0604/(sunang-0.0223)+0.0683
                        IF(difrat.lt.0.) difrat = 0.
                        IF(difrat.gt.1.) difrat = 1.

                        difrat = difrat+(1.0-difrat)*cloud
                        vnrat = (580.-cloud*464.)/((580.-cloud*499.)+(580.-cloud*464.))

                        forc_xy_sols %blk(ib,jb)%val(i,j) = a*(1.0-difrat)*vnrat
                        forc_xy_soll %blk(ib,jb)%val(i,j) = a*(1.0-difrat)*(1.0-vnrat)
                        forc_xy_solsd%blk(ib,jb)%val(i,j) = a*difrat*vnrat
                        forc_xy_solld%blk(ib,jb)%val(i,j) = a*difrat*(1.0-vnrat)
                     ENDDO
                  ENDDO
               ENDDO
            ENDIF
         ENDIF

         ! [GET ATMOSPHERE CO2 CONCENTRATION DATA]
         year  = idate(1)
         CALL julian2monthday (idate(1), idate(2), month, mday)
         pco2m = get_monthly_co2_mlo(year, month)*1.e-6
         CALL block_data_copy (forc_xy_pbot, forc_xy_pco2m, sca = pco2m        )
         CALL block_data_copy (forc_xy_pbot, forc_xy_po2m , sca = 0.209_r8     )

      ENDIF

      IF (.not. DEF_USE_Forcing_Downscaling) THEN

         ! Mapping the 2d atmospheric fields [lon_points]x[lat_points]
         !     -> the 1d vector of subgrid points [numpatch]
         CALL mg2p_forc_tracer%grid2pset (forc_xy_pco2m,  forc_pco2m)
         CALL mg2p_forc_tracer%grid2pset (forc_xy_po2m ,  forc_po2m )
         CALL mg2p_forc_tracer%grid2pset (forc_xy_us   ,  forc_us   )
         CALL mg2p_forc_tracer%grid2pset (forc_xy_vs   ,  forc_vs   )

         CALL mg2p_forc_tracer%grid2pset (forc_xy_psrf ,  forc_psrf )

         CALL mg2p_forc_tracer%grid2pset (forc_xy_sols ,  forc_sols )
         CALL mg2p_forc_tracer%grid2pset (forc_xy_soll ,  forc_soll )
         CALL mg2p_forc_tracer%grid2pset (forc_xy_solsd,  forc_solsd)
         CALL mg2p_forc_tracer%grid2pset (forc_xy_solld,  forc_solld)

         CALL mg2p_forc_tracer%grid2pset (forc_xy_hgt_t,  forc_hgt_t)
         CALL mg2p_forc_tracer%grid2pset (forc_xy_hgt_u,  forc_hgt_u)
         CALL mg2p_forc_tracer%grid2pset (forc_xy_hgt_q,  forc_hgt_q)

         IF (DEF_USE_CBL_HEIGHT) THEN
            CALL mg2p_forc_tracer%grid2pset (forc_xy_hpbl, forc_hpbl)
         ENDIF

         CALL mg2p_forc_tracer%grid2pset (forc_xy_t    ,  forc_t    )
         CALL mg2p_forc_tracer%grid2pset (forc_xy_q    ,  forc_q    )
         CALL mg2p_forc_tracer%grid2pset (forc_xy_prc  ,  forc_prc  )
         CALL mg2p_forc_tracer%grid2pset (forc_xy_prl  ,  forc_prl  )
         CALL mg2p_forc_tracer%grid2pset (forc_xy_pbot ,  forc_pbot )
         CALL mg2p_forc_tracer%grid2pset (forc_xy_frl  ,  forc_frl  )

         IF (p_is_worker) THEN

            DO np = 1, numpatch

               IF (.not. forcmask_pch_tracer(np)) CYCLE

               ! The standard measuring conditions for temperature are two meters above the ground
               ! Scientists have measured the most frigid temperature ever
               ! recorded on the continent's eastern highlands: about (180K) colder than dry ice.
               IF(forc_t(np) < 180.) forc_t(np) = 180.
               ! the highest air temp was found in Kuwait 326 K, Sulaibya 2012-07-31;
               ! Pakistan, Sindh 2010-05-26; Iraq, Nasiriyah 2011-08-03
               IF(forc_t(np) > 326.) forc_t(np) = 326.

               forc_rhoair(np) = (forc_pbot(np) &
                  - 0.378*forc_q(np)*forc_pbot(np)/(0.622+0.378*forc_q(np)))&
                  / (rgas*forc_t(np))

            ENDDO

         ENDIF

      ELSE
         ! ------------------------------------------------------
         ! Forcing downscaling module
         ! ------------------------------------------------------
         ! init forcing on patches
         CALL mg2p_forc_tracer%grid2pset (forc_xy_pco2m,   forc_pco2m)
         CALL mg2p_forc_tracer%grid2pset (forc_xy_po2m ,   forc_po2m )
         CALL mg2p_forc_tracer%grid2pset (forc_xy_us   ,   forc_us   )
         CALL mg2p_forc_tracer%grid2pset (forc_xy_vs   ,   forc_vs   )
         CALL mg2p_forc_tracer%grid2pset (forc_xy_psrf ,   forc_psrf )
         CALL mg2p_forc_tracer%grid2pset (forc_xy_sols ,   forc_sols )
         CALL mg2p_forc_tracer%grid2pset (forc_xy_soll ,   forc_soll )
         CALL mg2p_forc_tracer%grid2pset (forc_xy_solsd,   forc_solsd)
         CALL mg2p_forc_tracer%grid2pset (forc_xy_solld,   forc_solld)
         CALL mg2p_forc_tracer%grid2pset (forc_xy_solarin, forc_swrad)
         CALL mg2p_forc_tracer%grid2pset (forc_xy_hgt_t,   forc_hgt_t)
         CALL mg2p_forc_tracer%grid2pset (forc_xy_hgt_u,   forc_hgt_u)
         CALL mg2p_forc_tracer%grid2pset (forc_xy_hgt_q,   forc_hgt_q)

         IF (DEF_USE_CBL_HEIGHT) THEN
            CALL mg2p_forc_tracer%grid2pset (forc_xy_hpbl, forc_hpbl)
         ENDIF

         ! Mapping the 2d atmospheric fields [lon_points]x[lat_points]
         !     -> the 1d vector of subgrid points [numelm]
         !     by selected mapping methods
         CALL mg2p_forc_tracer%grid2part (forc_xy_t    ,   forc_t_grid    )
         CALL mg2p_forc_tracer%grid2part (forc_xy_q    ,   forc_q_grid    )
         CALL mg2p_forc_tracer%grid2part (forc_xy_prc  ,   forc_prc_grid  )
         CALL mg2p_forc_tracer%grid2part (forc_xy_prl  ,   forc_prl_grid  )
         CALL mg2p_forc_tracer%grid2part (forc_xy_pbot ,   forc_pbot_grid )
         CALL mg2p_forc_tracer%grid2part (forc_xy_frl  ,   forc_lwrad_grid)
         CALL mg2p_forc_tracer%grid2part (forc_xy_hgt_t,   forc_hgt_grid  )
         CALL mg2p_forc_tracer%grid2part (forc_xy_solarin, forc_swrad_grid)
         CALL mg2p_forc_tracer%grid2part (forc_xy_us,      forc_us_grid   )
         CALL mg2p_forc_tracer%grid2part (forc_xy_vs,      forc_vs_grid   )

         calday = calendarday(idate)

         IF (p_is_worker) THEN
            DO np = 1, numpatch ! patches

               ! calculate albedo of each patches
               IF (forc_sols(np)+forc_solsd(np)+forc_soll(np)+forc_solld(np) == 0) THEN
                  balb = 0
               ELSE
                  balb = ( alb(1,1,np)*forc_sols (np) + alb(1,2,np)*forc_solsd(np)   &
                         + alb(2,1,np)*forc_soll (np) + alb(2,2,np)*forc_solld(np) ) &
                       / (forc_sols(np)+forc_solsd(np)+forc_soll(np)+forc_solld(np))
               ENDIF

               DO ipart = 1, mg2p_forc_tracer%npart(np) ! part loop of each patch

                  IF (mg2p_forc_tracer%areapart(np)%val(ipart) == 0.) CYCLE

                  ! The standard measuring conditions for temperature are two meters above
                  ! the ground. Scientists have measured the most frigid temperature ever
                  ! recorded on the continent's eastern highlands: about (180K) colder than
                  ! dry ice.
                  IF (forc_t_grid(np)%val(ipart) < 180.) forc_t_grid(np)%val(ipart) = 180.
                  ! the highest air temp was found in Kuwait 326 K, Sulaibya 2012-07-31;
                  ! Pakistan, Sindh 2010-05-26; Iraq, Nasiriyah 2011-08-03
                  IF (forc_t_grid(np)%val(ipart) > 326.) forc_t_grid(np)%val(ipart) = 326.

                  forc_rho_grid(np)%val(ipart) = (forc_pbot_grid(np)%val(ipart) &
                     - 0.378*forc_q_grid(np)%val(ipart)*forc_pbot_grid(np)%val(ipart) &
                     /(0.622+0.378*forc_q_grid(np)%val(ipart)))/(rgas*forc_t_grid(np)%val(ipart))

                  forc_th_grid(np)%val(ipart) = forc_t_grid(np)%val(ipart) &
                     * (1.e5/forc_pbot_grid(np)%val(ipart)) ** (rair/cpair)

                  ! calculate sun zenith angle and sun azimuth angle and turn to degree
                  coszen(np) = orb_coszen(calday, patchlonr(np), patchlatr(np))
                  cosazi(np) = orb_cosazi(calday, patchlonr(np), patchlatr(np), coszen(np))

                  ! downscale forcing from grid to part
                  CALL downscale_forcings ( &
                     glacierss(np), &

                     ! non-adjusted forcing
                     forc_topo_grid(np)%val(ipart),  forc_maxelv_grid(np)%val(ipart), &
                     forc_t_grid(np)%val(ipart),     forc_th_grid(np)%val(ipart),     &
                     forc_q_grid(np)%val(ipart),     forc_pbot_grid(np)%val(ipart),   &
                     forc_rho_grid(np)%val(ipart),   forc_prc_grid(np)%val(ipart),    &
                     forc_prl_grid(np)%val(ipart),   forc_lwrad_grid(np)%val(ipart),  &
                     forc_hgt_grid(np)%val(ipart),   forc_swrad_grid(np)%val(ipart),  &
                     forc_us_grid(np)%val(ipart),    forc_vs_grid(np)%val(ipart),     &

                     ! topography-based factor on patch
                     slp_type_patches(:,np), asp_type_patches(:,np), area_type_patches(:,np), &
                     svf_patches(np), cur_patches(np), &
#ifdef SinglePoint
                     sf_lut_patches  (:,:,np), &
#else
                     sf_curve_patches(:,:,np), &
#endif

                     ! other factors
                     calday, coszen(np), cosazi(np), balb, &

                     ! adjusted forcing
                     forc_topo(np),                  forc_t_part(np)%val(ipart),      &
                     forc_th_part(np)%val(ipart),    forc_q_part(np)%val(ipart),      &
                     forc_pbot_part(np)%val(ipart),  forc_rhoair_part(np)%val(ipart), &
                     forc_prc_part(np)%val(ipart),   forc_prl_part(np)%val(ipart),    &

                     forc_frl_part(np)%val(ipart),   forc_swrad_part(np)%val(ipart),  &
                     forc_us_part(np)%val(ipart),    forc_vs_part(np)%val(ipart))
               ENDDO
            ENDDO
         ENDIF

         ! mapping parts to patches
         CALL mg2p_forc_tracer%part2pset (forc_t_part,      forc_t     )
         CALL mg2p_forc_tracer%part2pset (forc_q_part,      forc_q     )
         CALL mg2p_forc_tracer%part2pset (forc_pbot_part,   forc_pbot  )
         CALL mg2p_forc_tracer%part2pset (forc_rhoair_part, forc_rhoair)
         CALL mg2p_forc_tracer%part2pset (forc_prc_part,    forc_prc   )
         CALL mg2p_forc_tracer%part2pset (forc_prl_part,    forc_prl   )
         CALL mg2p_forc_tracer%part2pset (forc_frl_part,    forc_frl   )
         CALL mg2p_forc_tracer%part2pset (forc_swrad_part,  forc_swrad )
         CALL mg2p_forc_tracer%part2pset (forc_us_part,     forc_us    )
         CALL mg2p_forc_tracer%part2pset (forc_vs_part,     forc_vs    )

         ! wind downscaling
         IF (p_is_worker) THEN
            DO np = 1, numpatch
               IF ((forc_us(np)==spval).or.(forc_vs(np)==spval)) cycle
               CALL downscale_wind(forc_us(np), forc_vs(np), slp_type_patches(:,np), &
                        asp_type_patches(:,np), area_type_patches(:,np), cur_patches(np))
            ENDDO
         ENDIF

#ifndef SinglePoint
         IF (trim(DEF_DS_precipitation_adjust_scheme) == 'III') THEN
            ! Sisi Chen, Lu Li, Yongjiu Dai et al., 2024, JGR
            ! Using MPI to pass the forcing variable field to Python to
            ! accomplish precipitation downscaling
            IF (p_is_worker) THEN
               spaceship(1,1:numpatch) = forc_topo
               spaceship(2,1:numpatch) = forc_t
               spaceship(3,1:numpatch) = forc_pbot
               spaceship(4,1:numpatch) = forc_q
               spaceship(5,1:numpatch) = forc_frl
               spaceship(6,1:numpatch) = forc_swrad
               spaceship(7,1:numpatch) = forc_us
               spaceship(8,1:numpatch) = forc_vs
               spaceship(9,1:numpatch) = INT(calday)
               spaceship(10,1:numpatch) = patchlatr
               spaceship(11,1:numpatch) = patchlonr

               target_server = p_iam_glb/5+p_np_glb
               CALL MPI_SEND(spaceship,12*numpatch,MPI_REAL8,target_server,0,MPI_COMM_WORLD,ierr)
               CALL MPI_RECV(forc_prc,numpatch,MPI_REAL8,target_server,0,MPI_COMM_WORLD,&
                             MPI_STATUS_IGNORE,ierr)

               forc_prl = forc_prc/3600*2/3._r8
               forc_prc = forc_prc/3600*1/3._r8
            ENDIF

            ! mapping forc_prl to forc_prl_part, forc_prc to forc_prc_part
            IF (p_is_worker) THEN
               DO np = 1, numpatch ! patches
                  DO ipart = 1, mg2p_forc_tracer%npart(np) ! part loop of each patch
                     IF (mg2p_forc_tracer%areapart(np)%val(ipart) == 0.) CYCLE

                     forc_prl_part(np)%val(ipart) = forc_prl(np)
                     forc_prc_part(np)%val(ipart) = forc_prc(np)

                  ENDDO
               ENDDO
            ENDIF

            ! Conservation of convective and large scale precipitation in the grid of forcing
            CALL mg2p_forc_tracer%normalize (forc_xy_prc, forc_prc_part)
            CALL mg2p_forc_tracer%normalize (forc_xy_prl, forc_prl_part)

            ! mapping parts to patches
            CALL mg2p_forc_tracer%part2pset (forc_prc_part, forc_prc)
            CALL mg2p_forc_tracer%part2pset (forc_prl_part, forc_prl)
         ENDIF

         ! Conservation of short- and long- waves radiation in the grid of forcing
         CALL mg2p_forc_tracer%normalize (forc_xy_solarin, forc_swrad_part)
         CALL mg2p_forc_tracer%normalize (forc_xy_frl,     forc_frl_part  )
         CALL mg2p_forc_tracer%part2pset (forc_frl_part,    forc_frl   )
         CALL mg2p_forc_tracer%part2pset (forc_swrad_part,  forc_swrad )
#endif

         ! divide fractions of downscaled shortwave radiation
         IF (p_is_worker) THEN
            DO j = 1, numpatch
                  a = forc_swrad(j)
                  IF (isnan_ud(a)) a = 0
                  calday = calendarday(idate)
                  sunang = orb_coszen (calday, patchlonr(j), patchlatr(j))
                  IF (sunang.eq.0) THEN
                     cloud = 0.
                  ELSE
                     cloud = (1160.*sunang-a)/(963.*sunang)
                  ENDIF
                  cloud = max(cloud,0.0001)
                  cloud = min(cloud,1.)
                  cloud = max(0.58,cloud)

                  difrat = 0.0604/(sunang-0.0223)+0.0683
                  IF(difrat.lt.0.) difrat = 0.
                  IF(difrat.gt.1.) difrat = 1.

                  difrat = difrat+(1.0-difrat)*cloud
                  vnrat = (580.-cloud*464.)/((580.-cloud*499.)+(580.-cloud*464.))

                  forc_sols(j)  = a*(1.0-difrat)*vnrat
                  forc_soll(j)  = a*(1.0-difrat)*(1.0-vnrat)
                  forc_solsd(j) = a*difrat*vnrat
                  forc_solld(j) = a*difrat*(1.0-vnrat)
            ENDDO
         ENDIF
      ENDIF

#ifdef RangeCheck
#ifdef USEMPI
      CALL mpi_barrier (p_comm_glb, p_err)
#endif
      IF (p_is_master) write(*,'(/, A20)') 'Checking forcing ...'

      CALL check_vector_data ('Forcing us    [m/s]   ', forc_us   )
      CALL check_vector_data ('Forcing vs    [m/s]   ', forc_vs   )
      CALL check_vector_data ('Forcing t     [kelvin]', forc_t    )
      CALL check_vector_data ('Forcing q     [kg/kg] ', forc_q    )
      CALL check_vector_data ('Forcing prc   [mm/s]  ', forc_prc  )
      CALL check_vector_data ('Forcing psrf  [pa]    ', forc_psrf )
      CALL check_vector_data ('Forcing prl   [mm/s]  ', forc_prl  )
      CALL check_vector_data ('Forcing sols  [W/m2]  ', forc_sols )
      CALL check_vector_data ('Forcing soll  [W/m2]  ', forc_soll )
      CALL check_vector_data ('Forcing solsd [W/m2]  ', forc_solsd)
      CALL check_vector_data ('Forcing solld [W/m2]  ', forc_solld)
      CALL check_vector_data ('Forcing frl   [W/m2]  ', forc_frl  )
      IF (DEF_USE_CBL_HEIGHT) THEN
         CALL check_vector_data ('Forcing hpbl  ', forc_hpbl )
      ENDIF

#ifdef USEMPI
      CALL mpi_barrier (p_comm_glb, p_err)
#endif
#endif

   END SUBROUTINE read_tracer_forcing


!-----------------------------------------------------------------------
! !DESCRIPTION:
!  read lower and upper boundary forcing data, a major interface of this
!  MODULE
!
! !REVISIONS:
!  04/2014, Hua Yuan: initial code
!
!-----------------------------------------------------------------------
   SUBROUTINE metreadLBUB (idate, dir_forcing)

   USE MOD_UserSpecifiedForcing
   USE MOD_Namelist
   USE MOD_Block
   USE MOD_DataType
   USE MOD_Block
   USE MOD_NetCDFBlock
   USE MOD_RangeCheck
   IMPLICIT NONE

   integer, intent(in) :: idate(3)
   character(len=*), intent(in) :: dir_forcing

   ! Local variables
   integer         :: ivar, year, month, day, time_i
   integer         :: iblkme, ib, jb, i, j
   type(timestamp) :: mtstamp
   character(len=256) :: filename

      mtstamp = idate

      DO ivar = 1, NVAR_tracer

         IF (trim(tracer_vname(ivar)) == 'NULL') CYCLE     ! no data, CYCLE

         ! lower and upper boundary data already exist, CYCLE
         IF ( .not.(tstamp_LB_tracer(ivar)=='NULL') .and. .not.(tstamp_UB_tracer(ivar)=='NULL') .and. &
            tstamp_LB_tracer(ivar)<=mtstamp .and. mtstamp<tstamp_UB_tracer(ivar) ) THEN
            CYCLE
         ENDIF

         ! set lower boundary time stamp and get data
         IF (tstamp_LB_tracer(ivar) == 'NULL') THEN
            CALL setstampLB(mtstamp, ivar, year, month, day, time_i)

            ! read forcing data
            filename = trim(dir_forcing)//trim(metfilename(year, month, day, ivar))
            IF (trim(DEF_forcing%dataset) == 'POINT') THEN

               IF (tracer_read_ahead) THEN
                  tracerdata%blk(gblock%xblkme(1),gblock%yblkme(1))%val = tracer_disk(time_i,ivar)
               ELSE
#ifndef URBAN_MODEL
                  CALL ncio_read_site_time (filename, tracer_vname(ivar), time_i, tracerdata)
#else
                  IF (trim(tracer_vname(ivar)) == 'rainf_tracer') THEN
                     CALL ncio_read_site_time (filename, 'rainf_tracer', time_i, rainf_tracer)
                     CALL ncio_read_site_time (filename, 'snowf_tracer', time_i, snowf_tracer)

                     DO iblkme = 1, gblock%nblkme
                        ib = gblock%xblkme(iblkme)
                        jb = gblock%yblkme(iblkme)

                        tracerdata%blk(ib,jb)%val(1,1) = rainf_tracer%blk(ib,jb)%val(1,1) &
                                                    + snowf_tracer%blk(ib,jb)%val(1,1)
                     ENDDO
                  ELSE
                     CALL ncio_read_site_time (filename, tracer_vname(ivar), time_i, tracerdata)
                  ENDIF
#endif
               ENDIF
            ELSE
               CALL ncio_read_block_time (filename, tracer_vname(ivar), gforc_tracer, time_i, tracerdata)
            ENDIF

            CALL block_data_copy (tracerdata, forcn_tracer_LB(ivar))
         ENDIF

         ! set upper boundary time stamp and get data
         IF (tstamp_UB_tracer(ivar) == 'NULL' .or. tstamp_UB_tracer(ivar) <= mtstamp) THEN

            IF ( .not. (tstamp_UB_tracer(ivar) == 'NULL') ) THEN
               CALL block_data_copy (forcn_tracer_UB(ivar), forcn_tracer_LB(ivar))
            ENDIF

            CALL setstampUB(ivar, year, month, day, time_i)

            ! when reaching the END of forcing data, show a Warning but still try to run
            IF ( year>tracer_endyr .or. (month>tracer_endmo .and. year==tracer_endyr) ) THEN
               write(*,*) 'model year: ', year, 'forcing end year defined: ', tracer_endyr
               print *, 'Warning: reaching the END of forcing data defined!'
            ENDIF

            ! read forcing data
            filename = trim(dir_forcing)//trim(metfilename(year, month, day, ivar))
            IF (trim(DEF_forcing%dataset) == 'POINT') THEN

               IF (tracer_read_ahead) THEN
                  tracerdata%blk(gblock%xblkme(1),gblock%yblkme(1))%val = tracer_disk(time_i,ivar)
               ELSE
#ifndef URBAN_MODEL
                  CALL ncio_read_site_time (filename, tracer_vname(ivar), time_i, tracerdata)
#else
                  IF (trim(tracer_vname(ivar)) == 'rainf_tracer') THEN
                     CALL ncio_read_site_time (filename, 'rainf_tracer', time_i, rainf_tracer)
                     CALL ncio_read_site_time (filename, 'snowf_tracer', time_i, snowf_tracer)

                     DO iblkme = 1, gblock%nblkme
                        ib = gblock%xblkme(iblkme)
                        jb = gblock%yblkme(iblkme)

                        tracerdata%blk(ib,jb)%val(1,1) = rainf_tracer%blk(ib,jb)%val(1,1) &
                                                    + snowf_tracer%blk(ib,jb)%val(1,1)
                     ENDDO
                  ELSE
                     CALL ncio_read_site_time (filename, tracer_vname(ivar), time_i, tracerdata)
                  ENDIF
#endif
               ENDIF
            ELSE
               CALL ncio_read_block_time (filename, tracer_vname(ivar), gforc_tracer, time_i, tracerdata)
            ENDIF

            CALL block_data_copy (tracerdata, forcn_tracer_UB(ivar))

            ! calculate time average coszen, for shortwave radiation
            IF (ivar == 7) THEN
               CALL calavgcos_tracer(idate)
            ENDIF
         ENDIF

      ENDDO

   END SUBROUTINE metreadLBUB


!-----------------------------------------------------------------------
   SUBROUTINE tracer_read_latlon (dir_forcing, idate)

   USE MOD_SPMD_Task
   USE MOD_NetCDFSerial
   USE MOD_UserSpecifiedForcing
   USE MOD_Namelist
   IMPLICIT NONE

   character(len=*), intent(in) :: dir_forcing
   integer, intent(in) :: idate(3)

   ! Local variables
   character(len=256) :: filename
   integer         :: year, month, day, time_i
   type(timestamp) :: mtstamp
   real(r8), allocatable :: latxy (:,:)    ! latitude values in 2d
   real(r8), allocatable :: lonxy (:,:)    ! longitude values in 2d
   real(r8), allocatable :: lon_in(:)
   real(r8), allocatable :: lat_in(:)

      IF (trim(DEF_forcing%dataset) == 'POINT' .or. trim(DEF_forcing%dataset) == 'CPL7' ) THEN
         CALL gforc_tracer%define_by_ndims (360, 180)
      ELSE

         mtstamp = idate

         CALL setstampLB(mtstamp, 1, year, month, day, time_i)
         filename = trim(dir_forcing)//trim(metfilename(year, month, day, 1))
         tstamp_LB_tracer(1) = timestamp(-1, -1, -1)

         IF (tracer_dim2d) THEN
            CALL ncio_read_bcast_serial (filename, latname, latxy)
            CALL ncio_read_bcast_serial (filename, lonname, lonxy)

            allocate (lat_in (size(latxy,2)))
            allocate (lon_in (size(lonxy,1)))
            lat_in = latxy(1,:)
            lon_in = lonxy(:,1)

            deallocate (latxy)
            deallocate (lonxy)
         ELSE
            CALL ncio_read_bcast_serial (filename, latname, lat_in)
            CALL ncio_read_bcast_serial (filename, lonname, lon_in)
         ENDIF

         IF (.not. DEF_forcing%tracer_regional) THEN
            CALL gforc_tracer%define_by_center (lat_in, lon_in)
         ELSE
            CALL gforc_tracer%define_by_center (lat_in, lon_in, &
               south = DEF_forcing%tracer_regbnd(1), north = DEF_forcing%tracer_regbnd(2), &
               west  = DEF_forcing%tracer_regbnd(3), east  = DEF_forcing%tracer_regbnd(4))
         ENDIF

         deallocate (lat_in)
         deallocate (lon_in)
      ENDIF

      CALL gforc_tracer%set_rlon ()
      CALL gforc_tracer%set_rlat ()

   END SUBROUTINE tracer_read_latlon

!-----------------------------------------------------------------------
   SUBROUTINE metread_time (dir_forcing, ststamp, etstamp, deltime)

   USE MOD_SPMD_Task
   USE MOD_NetCDFSerial
   USE MOD_UserSpecifiedForcing
   USE MOD_Namelist
   IMPLICIT NONE

   character(len=*), intent(in) :: dir_forcing
   type(timestamp),  intent(in), optional :: ststamp, etstamp
   real(r8),         intent(in), optional :: deltime

   ! Local variables
   character(len=256) :: filename
   character(len=256) :: timeunit, timestr
   real(r8), allocatable :: forctime_sec(:), metcache(:,:,:)
   integer :: year, month, day, hour, minute, second
   integer :: itime, maxday, id(3)
   integer*8 :: sec_long
   integer :: ivar, ntime, its, ite, it
   real(r8) firstsec

   type(timestamp) :: etstamp_f
   type(timestamp), allocatable :: forctime_ (:)


      filename = trim(dir_forcing)//trim(tracer_fprefix(1))

      CALL ncio_read_serial (filename, 'time', forctime_sec)
      CALL ncio_get_attr    (filename, 'time', 'units', timeunit)

      timestr = timeunit(15:18) // ' ' // timeunit(20:21) // ' ' // timeunit(23:24) &
         // ' ' // timeunit(26:27) // ' ' // timeunit(29:30) // ' ' // timeunit(32:33)
      read(timestr,*) year, month, day, hour, minute, second

      allocate (forctime (size(forctime_sec)))

      id(1) = year
      id(2) = get_calday(month*100+day, isleapyear(year))
      id(3) = hour*3600 + minute*60 + second

      firstsec = forctime_sec(1)
      DO WHILE (firstsec > 86400)
         CALL ticktime (86400., id)
         firstsec = firstsec - 86400
      ENDDO
      CALL ticktime (firstsec, id)

      !forctime(1)%year = year
      !forctime(1)%day  = get_calday(month*100+day, isleapyear(year))
      !forctime(1)%sec  = hour*3600 + minute*60 + second + forctime_sec(1)

      !id(:) = (/forctime(1)%year, forctime(1)%day, forctime(1)%sec/)
      CALL adj2end(id)
      forctime(1) = id

      ntime = size(forctime)

      DO itime = 2, ntime
         id(:) = (/forctime(itime-1)%year, forctime(itime-1)%day, forctime(itime-1)%sec/)
         CALL ticktime (forctime_sec(itime)-forctime_sec(itime-1), id)
         forctime(itime) = id
      ENDDO

      IF (tracer_read_ahead) THEN

         CALL ticktime (deltime, id)
         etstamp_f = id

         IF ((ststamp < forctime(1)) .or. (etstamp_f < etstamp)) THEN
            write(*,*) 'Error: Forcing does not cover simulation period!'
            write(*,*) 'Model start ', ststamp,     ' -> Model END ', etstamp
            write(*,*) 'Forc  start ', forctime(1), ' -> Forc END  ', etstamp_f
            CALL CoLM_stop ()
         ELSE
            its = 1
            DO WHILE (.not. (ststamp < forctime(its+1)))
               its = its + 1
               IF (its >= ntime) EXIT
            ENDDO

            ite = ntime
            DO WHILE (etstamp < forctime(ite-1))
               ite = ite - 1
               IF (ite <= 1) EXIT
            ENDDO

            ntime = ite-its+1

            allocate (forctime_(ntime))
            DO it = 1, ntime
               forctime_(it) = forctime(it+its-1)
            ENDDO

            deallocate (forctime)
            allocate (forctime (ntime))
            DO it = 1, ntime
               forctime(it) = forctime_(it)
            ENDDO

            deallocate(forctime_)
         ENDIF

         allocate (tracer_disk (size(forctime),NVAR_tracer))

         filename = trim(dir_forcing)//trim(metfilename(-1,-1,-1,-1))
         DO ivar = 1, NVAR_tracer
            IF (trim(tracer_vname(ivar)) /= 'NULL') THEN
#ifndef URBAN_MODEL
               CALL ncio_read_period_serial (filename, tracer_vname(ivar), its, ite, metcache)
               tracer_disk(:,ivar) = metcache(1,1,:)
#else
               IF (trim(tracer_vname(ivar)) == 'rainf_tracer') THEN
                  CALL ncio_read_period_serial (filename, 'rainf_tracer', its, ite, metcache)
                  tracer_disk(:,ivar) = metcache(1,1,:)

                  CALL ncio_read_period_serial (filename, 'snowf_tracer', its, ite, metcache)
                  tracer_disk(:,ivar) = tracer_disk(:,ivar) + metcache(1,1,:)
               ELSE
                  CALL ncio_read_period_serial (filename, tracer_vname(ivar), its, ite, metcache)
                  tracer_disk(:,ivar) = metcache(1,1,:)
               ENDIF
#endif
            ENDIF
         ENDDO

         IF (allocated(metcache)) deallocate(metcache)
      ENDIF

   END SUBROUTINE metread_time

!-----------------------------------------------------------------------
! !DESCRIPTION:
!    set the lower boundary time stamp and record information,
!    a KEY FUNCTION of this MODULE
!
! - for time stamp, set it regularly as the model time step.
! - for record information, account for:
!    o year alternation
!    o month alternation
!    o leap year
!    o required data just beyond the first record
!
! !REVISIONS:
!  04/2014, Hua Yuan: initial code
!
!-----------------------------------------------------------------------
   SUBROUTINE setstampLB(mtstamp, var_i, year, month, mday, time_i)

   IMPLICIT NONE
   type(timestamp), intent(in)  :: mtstamp
   integer,         intent(in)  :: var_i
   integer,         intent(out) :: year
   integer,         intent(out) :: month
   integer,         intent(out) :: mday
   integer,         intent(out) :: time_i

   integer :: i, day, sec, ntime
   integer :: months(0:12)

      year = mtstamp%year
      day  = mtstamp%day
      sec  = mtstamp%sec

      IF (trim(DEF_forcing%dataset) == 'POINT') THEN

         ntime  = size(forctime)
         time_i = 1

         IF ((mtstamp < forctime(1)) .or. (forctime(ntime) < mtstamp)) THEN
            write(*,*) 'Error: Forcing does not cover simulation period!'
            write(*,*) 'Need ', mtstamp, ', Forc start ', forctime(1), ', Forc END', forctime(ntime)
            CALL CoLM_stop ()
         ELSE
            DO WHILE (.not. (mtstamp < forctime(time_i+1)))
               time_i = time_i + 1
            ENDDO
            iforctime(var_i) = time_i
            tstamp_LB_tracer(var_i) = forctime(iforctime(var_i))
         ENDIF

         RETURN
      ENDIF

      tstamp_LB_tracer(var_i)%year = year
      tstamp_LB_tracer(var_i)%day  = day

      ! in the case of one year one file
      IF ( trim(tracer_groupby) == 'year' ) THEN

         ! calculate the initial second
         sec    = 86400*(day-1) + sec
         time_i = floor( (sec-tracer_offset(var_i)) *1. / tracer_dtime(var_i) ) + 1
         sec    = (time_i-1)*tracer_dtime(var_i) + tracer_offset(var_i) - 86400*(day-1)
         tstamp_LB_tracer(var_i)%sec = sec

         ! set time stamp (ststamp_LB_tracer)
         IF (sec < 0) THEN
            tstamp_LB_tracer(var_i)%sec = 86400 + sec
            tstamp_LB_tracer(var_i)%day = day - 1
            IF (tstamp_LB_tracer(var_i)%day == 0) THEN
               tstamp_LB_tracer(var_i)%year = year - 1
               IF ( isleapyear(tstamp_LB_tracer(var_i)%year) ) THEN
                  tstamp_LB_tracer(var_i)%day = 366
               ELSE
                  tstamp_LB_tracer(var_i)%day = 365
               ENDIF
            ENDIF
         ENDIF

         ! set record info (year, time_i)
         IF ( sec<0 .or. (sec==0 .and. tracer_offset(var_i).NE.0) ) THEN

            ! IF the required data just behind the first record
            ! -> set to the first record
            IF ( year==tracer_startyr .and. month==tracer_startmo .and. day==1 ) THEN
               sec = tracer_offset(var_i)

               ! ELSE, set to one record backward
            ELSE
               sec = 86400 + sec
               day = day - 1
               IF (day == 0) THEN
                  year = year - 1
                  IF ( isleapyear(year) ) THEN
                     day = 366
                  ELSE
                     day = 365
                  ENDIF
               ENDIF
            ENDIF
         ENDIF ! ENDIF (sec <= 0)

         ! in case of tracer_leapyear with a non-tracer_leapyear calendar
         ! USE the data 1 day before after FEB 28th (Julian day 59).
         IF ( .not. tracer_leapyear .and. isleapyear(year) .and. day>59 ) THEN
            day = day - 1
         ENDIF

         ! get record time index
         sec = 86400*(day-1) + sec
         time_i = floor( (sec-tracer_offset(var_i)) *1. / tracer_dtime(var_i) ) + 1
      ENDIF

      ! in the case of one month one file
      IF ( trim(tracer_groupby) == 'month' ) THEN

         IF ( isleapyear(year) ) THEN
            months = (/0,31,60,91,121,152,182,213,244,274,305,335,366/)
         ELSE
            months = (/0,31,59,90,120,151,181,212,243,273,304,334,365/)
         ENDIF

         ! calculate initial month and day values
         CALL julian2monthday(year, day, month, mday)

         ! calculate initial second value
         sec    = 86400*(mday-1) + sec
         time_i = floor( (sec-tracer_offset(var_i)) *1. / tracer_dtime(var_i) ) + 1
         sec    = (time_i-1)*tracer_dtime(var_i) + tracer_offset(var_i) - 86400*(mday-1)
         tstamp_LB_tracer(var_i)%sec  = sec

         ! set time stamp (ststamp_LB_tracer)
         IF (sec < 0) THEN
            tstamp_LB_tracer(var_i)%sec = 86400 + sec
            tstamp_LB_tracer(var_i)%day = day - 1
            IF (tstamp_LB_tracer(var_i)%day == 0) THEN
               tstamp_LB_tracer(var_i)%year = year - 1
               IF ( isleapyear(tstamp_LB_tracer(var_i)%year) ) THEN
                  tstamp_LB_tracer(var_i)%day = 366
               ELSE
                  tstamp_LB_tracer(var_i)%day = 365
               ENDIF
            ENDIF
         ENDIF

         ! set record info (year, month, time_i)
         IF ( sec<0 .or. (sec==0 .and. tracer_offset(var_i).ne.0) ) THEN

            ! IF just behind the first record -> set to first record
            IF ( year==tracer_startyr .and. month==tracer_startmo .and. mday==1 ) THEN
               sec = tracer_offset(var_i)

               ! set to one record backward
            ELSE
               sec = 86400 + sec
               mday = mday - 1
               IF (mday == 0) THEN
                  month = month - 1
                  ! bug found by Zhu Siguang & Zhang Xiangxiang, 05/19/2014
                  ! move the below line in the 'ELSE' statement
                  !mday = months(month) - months(month-1)
                  IF (month == 0) THEN
                     month = 12
                     year = year - 1
                     mday = 31
                  ELSE
                     mday = months(month) - months(month-1)
                  ENDIF
               ENDIF
            ENDIF
         ENDIF

         ! in case of tracer_leapyear with a non-tracer_leapyear calendar
         ! USE the data 1 day before, i.e., FEB 28th.
         IF ( .not. tracer_leapyear .and. isleapyear(year) .and. month==2 .and. mday==29 ) THEN
            mday = 28
         ENDIF

         ! get record time index
         sec = 86400*(mday-1) + sec
         time_i = floor( (sec-tracer_offset(var_i)) *1. / tracer_dtime(var_i) ) + 1
      ENDIF

      ! in the case of one day one file
      IF ( trim(tracer_groupby) == 'day' ) THEN

         ! calculate initial month and day values
         CALL julian2monthday(year, day, month, mday)

         ! calculate initial second value
         time_i = floor( (sec-tracer_offset(var_i)) *1. / tracer_dtime(var_i) ) + 1
         sec    = (time_i-1)*tracer_dtime(var_i) + tracer_offset(var_i)
         tstamp_LB_tracer(var_i)%sec  = sec

         ! set time stamp (ststamp_LB_tracer)
         IF (sec < 0) THEN
            tstamp_LB_tracer(var_i)%sec = 86400 + sec
            tstamp_LB_tracer(var_i)%day = day - 1
            IF (tstamp_LB_tracer(var_i)%day == 0) THEN
               tstamp_LB_tracer(var_i)%year = year - 1
               IF ( isleapyear(tstamp_LB_tracer(var_i)%year) ) THEN
                  tstamp_LB_tracer(var_i)%day = 366
               ELSE
                  tstamp_LB_tracer(var_i)%day = 365
               ENDIF
            ENDIF

            IF ( year==tracer_startyr .and. month==tracer_startmo .and. mday==1 ) THEN
               sec = tracer_offset(var_i)
            ! set to one record backward
            ELSE
               sec = 86400 + sec
               year = tstamp_LB_tracer(var_i)%year
               CALL julian2monthday(tstamp_LB_tracer(var_i)%year, tstamp_LB_tracer(var_i)%day, month, mday)
            ENDIF
         ENDIF

         ! in case of tracer_leapyear with a non-tracer_leapyear calendar
         ! USE the data 1 day before, i.e., FEB 28th.
         IF ( .not. tracer_leapyear .and. isleapyear(year) .and. month==2 .and. mday==29 ) THEN
            mday = 28
         ENDIF

         ! get record time index
         time_i = floor( (sec-tracer_offset(var_i)) *1. / tracer_dtime(var_i) ) + 1
      ENDIF

      IF (time_i <= 0) THEN
         write(6, *) "got the wrong time record of forcing! STOP!"; CALL CoLM_stop()
      ENDIF

      RETURN

   END SUBROUTINE setstampLB

!-----------------------------------------------------------------------
! !DESCRIPTION:
!    set the upper boundary time stamp and record information,
!    a KEY FUNCTION of this MODULE
!
! !REVISIONS:
!  04/2014, Hua Yuan: initial code
!
!-----------------------------------------------------------------------
   SUBROUTINE setstampUB(var_i, year, month, mday, time_i)

   IMPLICIT NONE
   integer,         intent(in)  :: var_i
   integer,         intent(out) :: year
   integer,         intent(out) :: month
   integer,         intent(out) :: mday
   integer,         intent(out) :: time_i

   integer :: day, sec
   integer :: months(0:12)

      IF (trim(DEF_forcing%dataset) == 'POINT') THEN
         IF ( tstamp_UB_tracer(var_i) == 'NULL' ) THEN
            tstamp_UB_tracer(var_i) = forctime(iforctime(var_i)+1)
         ELSE
            iforctime(var_i) = iforctime(var_i) + 1
            tstamp_LB_tracer(var_i) = forctime(iforctime(var_i))
            tstamp_UB_tracer(var_i) = forctime(iforctime(var_i)+1)
         ENDIF

         time_i = iforctime(var_i)+1
         year = tstamp_UB_tracer(var_i)%year
         RETURN
      ENDIF

      ! calculate the time stamp
      IF ( tstamp_UB_tracer(var_i) == 'NULL' ) THEN
         tstamp_UB_tracer(var_i) = tstamp_LB_tracer(var_i) + tracer_dtime(var_i)
      ELSE
         tstamp_LB_tracer(var_i) = tstamp_UB_tracer(var_i)
         tstamp_UB_tracer(var_i) = tstamp_UB_tracer(var_i) + tracer_dtime(var_i)
      ENDIF

      ! calculate initial year, day, and second values
      year = tstamp_UB_tracer(var_i)%year
      day  = tstamp_UB_tracer(var_i)%day
      sec  = tstamp_UB_tracer(var_i)%sec

      IF ( trim(tracer_groupby) == 'year' ) THEN

         ! adjust year value
         IF ( sec==86400 .and. tracer_offset(var_i).eq.0 ) THEN
            sec = 0
            day = day + 1
            IF( isleapyear(year) .and. day==367) THEN
               year = year + 1; day = 1
            ENDIF
            IF( .not. isleapyear(year) .and. day==366) THEN
               year = year + 1; day = 1
            ENDIF
         ENDIF

         ! in case of tracer_leapyear with a non-tracer_leapyear calendar
         ! USE the data 1 day before after FEB 28th (Julian day 59).
         IF ( .not. tracer_leapyear .and. isleapyear(year) .and. day>59 ) THEN
            day = day - 1
         ENDIF

         ! set record index
         sec = 86400*(day-1) + sec
         time_i = floor( (sec-tracer_offset(var_i)) *1. / tracer_dtime(var_i) ) + 1
      ENDIF

      IF ( trim(tracer_groupby) == 'month' ) THEN

         IF ( isleapyear(year) ) THEN
            months = (/0,31,60,91,121,152,182,213,244,274,305,335,366/)
         ELSE
            months = (/0,31,59,90,120,151,181,212,243,273,304,334,365/)
         ENDIF

         ! calculate initial month and day values
         CALL julian2monthday(year, day, month, mday)

         ! record in the next day, adjust year, month and second values
         IF ( sec==86400 .and. tracer_offset(var_i).eq.0 ) THEN
            sec  = 0
            mday = mday + 1
            IF ( mday > (months(month)-months(month-1)) ) THEN
               mday = 1
               ! bug found by Zhu Siguang, 05/25/2014
               ! move the below line in the 'ELSE' statement
               !month = month + 1
               IF (month == 12) THEN
                  month = 1
                  year = year + 1
               ELSE
                  month = month + 1
               ENDIF
            ENDIF
         ENDIF

         ! in case of tracer_leapyear with a non-tracer_leapyear calendar
         ! for day 29th Feb, USE the data 1 day before, i.e., 28th FEB.
         IF ( .not. tracer_leapyear .and. isleapyear(year) .and. month==2 .and. mday==29 ) THEN
            mday = 28
         ENDIF

         ! set record index
         sec    = 86400*(mday-1) + sec
         time_i = floor( (sec-tracer_offset(var_i)) *1. / tracer_dtime(var_i) ) + 1
      ENDIF

      IF ( trim(tracer_groupby) == 'day' ) THEN
         IF ( isleapyear(year) ) THEN
            months = (/0,31,60,91,121,152,182,213,244,274,305,335,366/)
         ELSE
            months = (/0,31,59,90,120,151,181,212,243,273,304,334,365/)
         ENDIF

         ! calculate initial month and day values
         CALL julian2monthday(year, day, month, mday)
         !mday = day

         ! record in the next day, adjust year, month and second values
         IF ( sec==86400 .and. tracer_offset(var_i).eq.0 ) THEN
            sec  = 0
            mday = mday + 1
            IF ( mday > (months(month)-months(month-1)) ) THEN
               mday = 1
               ! bug found by Zhu Siguang, 05/25/2014
               ! move the below line in the 'ELSE' statement
               !month = month + 1
               IF (month == 12) THEN
                  month = 1
                  year = year + 1
               ELSE
                  month = month + 1
               ENDIF
            ENDIF
         ENDIF

         ! in case of tracer_leapyear with a non-tracer_leapyear calendar
         ! for day 29th Feb, USE the data 1 day before, i.e., 28th FEB.
         IF ( .not. tracer_leapyear .and. isleapyear(year) .and. month==2 .and. mday==29 ) THEN
            mday = 28
         ENDIF

         ! set record index
         time_i = floor( (sec-tracer_offset(var_i)) *1. / tracer_dtime(var_i) ) + 1
      ENDIF

      IF (time_i < 0) THEN
         write(6, *) "got the wrong time record of forcing! STOP!"; CALL CoLM_stop()
      ENDIF

      RETURN

   END SUBROUTINE setstampUB

!-----------------------------------------------------------------------
! !DESCRIPTION:
!  calculate time average coszen value between [LB, UB]
!
! !REVISIONS:
!  04/2014, Hua Yuan: this method is adapted from CLM
!
!-----------------------------------------------------------------------
   SUBROUTINE calavgcos_tracer(idate)

   USE MOD_Block
   USE MOD_DataType
   IMPLICIT NONE

   integer, intent(in) :: idate(3)

   integer  :: ntime, iblkme, ib, jb, i, j, ilon, ilat
   real(r8) :: calday, cosz
   type(timestamp) :: tstamp

      tstamp = idate !tstamp_LB_tracer(7)
      ntime = 0
      DO WHILE (tstamp < tstamp_UB_tracer(7))
         ntime  = ntime + 1
         tstamp = tstamp + deltim_int_tracer
      ENDDO

      tstamp = idate !tstamp_LB_tracer(7)
      CALL flush_block_data (avgcos_tracer, 0._r8)

      DO WHILE (tstamp < tstamp_UB_tracer(7))

         DO iblkme = 1, gblock%nblkme
            ib = gblock%xblkme(iblkme)
            jb = gblock%yblkme(iblkme)
            DO j = 1, gforc_tracer%ycnt(jb)
               DO i = 1, gforc_tracer%xcnt(ib)

                  ilat = gforc_tracer%ydsp(jb) + j
                  ilon = gforc_tracer%xdsp(ib) + i
                  IF (ilon > gforc_tracer%nlon) ilon = ilon - gforc_tracer%nlon

                  calday = calendarday(tstamp)
                  cosz = orb_coszen(calday, gforc_tracer%rlon(ilon), gforc_tracer%rlat(ilat))
                  cosz = max(0.001, cosz)
                  avgcos_tracer%blk(ib,jb)%val(i,j) = avgcos_tracer%blk(ib,jb)%val(i,j) &
                     + cosz / real(ntime,r8) !  * deltim_real /real(tstamp_UB_tracer(7)-tstamp_LB_tracer(7))

               ENDDO
            ENDDO
         ENDDO

         tstamp = tstamp + deltim_int_tracer

      ENDDO

   END SUBROUTINE calavgcos_tracer

END MODULE MOD_Tracer_Forcing
