...
!-------------------------------------------------------------------------------------
   ...
   real(r8), parameter :: inp_cap_factor = 1._r8
	   ...
	   real(r8), allocatable :: a_trc_storage_mass(:,:) ! Accumulated storage tracer [mass * s]
	   real(r8), allocatable :: a_water_storage(:)       ! Accumulated water storage [m3 * s]
...
CONTAINS
      ...
      ENDIF
	      ...
	      ELSE
            ...
            ENDIF
	         ...
	         IF (has_levee_cell .and. allocated(levsto)) THEN
   ...
   USE MOD_Grid_RiverLakeLevee, only: has_levee, volwater_ucat, levsto
	   ...
	   USE MOD_Tracer_Defs, only: trc_tiny, tracer_init_water_ratio, tracer_can_use_fixed_signature, &
   ...
   integer,  intent(in) :: npthout_local_in
	   ...
	   real(r8) :: volwater, volwater_next, dt_i, dt_donor, trc_pth_fl
	   ...
	   real(r8) :: trc_inj_tau, m_cap, m_room, m_tau, release, R_cap, R_fill
      ...
      ENDIF
	      ...
	      DO itrc = 1, ntracers
	         ...
	         ENDIF
		         ...
		         ! --- 1. Inject this routing-period runoff tracer with the water input ---
	         ! Keep a per-tracer copy for the buffer release cap below. The
	         ! accumulator itself must be cleared after injection, but using it
	         ! after clearing made the fresh-input branch unreachable and forced
	         ! residual buffer release to fall back to R_init.
	         trc_inp_step(:) = acc_trc_inp(itrc, :)
	         DO i = 1, numucat
	            trc_mass(itrc, i) = trc_mass(itrc, i) + trc_inp_step(i)
	            acc_trc_inp(itrc, i) = 0._r8
	         ENDDO
         ...
         DO i = 1, numucat
            ...
            IF (volwater > trc_v_dry_off) THEN
	               ...
	               IF (trc_inp_step(i) > trc_tiny .and. acc_rnof_ref(i) > trc_tiny) THEN
	                  m_cap = inp_cap_factor * (trc_inp_step(i) / max(acc_rnof_ref(i), 1.e-30_r8)) * volwater
	               ...
	               ELSE
	                  ...
	                  R_cap = max(trc_mass(itrc, i), 0._r8) / max(volwater, trc_v_dry_off)
         ...
         ! Bifurcation gross incoming mass. This uses the signed net after
         ! rebuilding pathway exchanges; it is sufficient to preserve the
         ! uniform-tracer invariant while keeping the limiter conservative.
         DO i = 1, numucat
            IF (irivsys(i) > 0 .and. irivsys(i) <= size(dt_all)) THEN
               dt_i = dt_all(irivsys(i))
            ELSE
            ...
            ENDIF
            IF (dt_i <= 0._r8) CYCLE
            IF (bif_net(i) < 0._r8) trc_in_mass(i) = trc_in_mass(i) - bif_net(i) * dt_i
            IF (trc_bif_lev_net(i) < 0._r8) trc_in_mass_lev(i) = -trc_bif_lev_net(i) * dt_i
         ENDDO
         ...
         DO i = 1, numucat
            ...
            IF (volwater_next > trc_v_dry_off) THEN
               ...
               ratio_snap_tol = max(abs(R_fill) * fixed_sig_rel_tol, trc_tiny)
	               IF (fixed_signature_transport .and. &
   ...
   logical, optional, intent(out) :: missing_mask(:)
      ...
      DO itrc = 1, ntracers
         ...
         ELSE
            ...
            USE MOD_Grid_RiverLakeNetwork, only: lake_type_bf => lake_type
            ...
            USE MOD_Tracer_Defs,         only: tracer_init_water_ratio_bf => tracer_init_water_ratio
            integer  :: ii_bf
            ...
            IF (DEF_USE_LEVEE .and. p_is_worker .and. numucat > 0 &
               ...
               R_bf = tracer_init_water_ratio_bf(itrc)
   ...
   real(r8), intent(in) :: acctime_hist  ! Total accumulated history time [s]
	   ...
	   real(r8), allocatable :: tmpvec(:)
	   real(r8), allocatable :: volresv_hist(:)
	   integer,  allocatable :: ucat2resv_hist(:)
	   real(r8) :: volwater, ratio_loc, delta_loc
      ...
      IF (acctime_hist <= 0._r8) RETURN
	      ...
	      IF (p_is_worker) THEN
	         IF (allocated(volresv_state)) THEN
	            allocate (volresv_hist(size(volresv_state)))
	            volresv_hist(:) = volresv_state(:)
	         ELSE
	            allocate (volresv_hist(0))
	         ENDIF
	         IF (allocated(ucat2resv_state)) THEN
	            allocate (ucat2resv_hist(size(ucat2resv_state)))
	            ucat2resv_hist(:) = ucat2resv_state(:)
	         ELSE
	            allocate (ucat2resv_hist(0))
	         ENDIF
	      ELSE
	         allocate (volresv_hist(0))
	         allocate (ucat2resv_hist(0))
	               ...
	               IF (tracers(itrc)%ref_ratio > trc_tiny) THEN
	                  DO i = 1, numucat
	                     ...
	                     ENDIF
		                     ...
		                     IF (a_water_storage(i) <= trc_delta_diag_vmin * acctime_hist) CYCLE
      ...
      END BLOCK
	      ...
	      deallocate (tmpvec)
	      deallocate (volresv_hist)
	      deallocate (ucat2resv_hist)
